...
class AbSpleen(AbOrgan):

    model = None

    def __init__(self):
        super().__init__()
        self.logger = logging.getLogger('AbSpleen')
        self.logger.setLevel(logging.DEBUG)
        self.region=None

    def HasModel(self):
        if AbSpleen.model is None:
            return(False)
        else:
            return(True)
...
def main():
    ...
    my_parser = argparse.ArgumentParser(description='Identify spleen')
    ...
    AbSpleen.model =  tf.keras.models.load_model(args.model, custom_objects=get_jabba_custom_objects())
